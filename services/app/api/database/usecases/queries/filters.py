...
class EqualFilter(Filter):
    ...
    def __filter(self, query: str, query_parameters: dict[str, dict[str, Any]]) -> tuple[str, dict[str, dict[str, Any]]]:
    ...
    def __filter(self, query: str, query_parameters: dict[str, dict[str, Any]]) -> tuple[str, dict[str, dict[str, Any]]]:
        ...
        for field_name, field_parameters in query_parameters.items():
            ...
            if field_parameters.get('lt') and field_parameters.get('gt'):
                ...
                if "WHERE" in query:
                    query = query + f" AND {field_name} {gt_operator} {gt_value} AND {field_name} {lt_operator} {lt_value}"
                else:
                    query = query + f" WHERE {field_name} {gt_operator} {gt_value} AND {field_name} {lt_operator} {lt_value}"
        for field in fields_to_drop:
            query_parameters.pop(field)
        return query, query_parameters
