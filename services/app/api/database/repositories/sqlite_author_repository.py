...
class SQLiteCommentAuthorRepository(BaseRepository[CommentAuthor]):
    ...
    def add(self, comment_author: CommentAuthor) -> CommentAuthor:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?)
            """,
            (comment_author.author_display_name, comment_author.author_profile_image_url, 
             comment_author.author_channel_url, comment_author.author_channel_id)
            )
    ...
    def update(self, comment_author: CommentAuthor) -> CommentAuthor:
        ...
        """,
        (comment_author.author_display_name, comment_author.author_profile_image_url, 
         comment_author.author_channel_url, comment_author.author_channel_id, comment_author.author_id)
        )
        return comment_author
    ...
    def delete(self, author_id: int) -> None:
        cursor = self.connection.cursor()
        cursor.execute(
            """DELETE FROM comment_authors WHERE author_id=?""",
            (author_id,)
        )
