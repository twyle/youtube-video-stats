...
class SQLitePlaylistRepository(BaseRepository[Playlist]):
    ...
    def add(self, playlist: Playlist) -> Playlist:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?,?)
            """,
            (playlist.playlist_id, playlist.playlist_title, playlist.playlist_description, 
             playlist.playlist_thumbnail, playlist.videos_count, playlist.published_at, 
             playlist.channel_id)
            )
    ...
    def update(self, playlist: Playlist) -> Playlist:
        ...
        (playlist.playlist_id, playlist.playlist_title, playlist.playlist_description, 
         playlist.playlist_thumbnail, playlist.videos_count, playlist.published_at, 
         playlist.channel_id ,playlist.id)
        )
        return playlist
    ...
    def list_all(self, limit: Optional[int] = 2, sort_order: Optional[str] = 'ASC', 
                 sort_field: Optional[str] = 'id', offset: Optional[int] = 0) -> list[Playlist]:
        cursor = self.connection.cursor()
        cursor.execute(
        f"""
         SELECT id, playlist_id, playlist_title, playlist_description, playlist_thumbnail, 
