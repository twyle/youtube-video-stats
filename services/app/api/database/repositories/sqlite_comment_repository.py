...
class SQLiteCommentRepository(BaseRepository[Comment]):
    ...
    def add(self, comment: Comment) -> Comment:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (comment.video_id, comment.comment_id, comment.comment_text, 
             comment.parent_id, comment.like_count, comment.published_at, 
             comment.updated_at)
            )
    ...
    def list_all(self, limit: Optional[int] = 2, sort_order: Optional[str] = 'ASC', 
                 sort_field: Optional[str] = 'id', offset: Optional[int] = 0) -> list[Comment]:
        cursor = self.connection.cursor()
        cursor.execute(
        f"""
