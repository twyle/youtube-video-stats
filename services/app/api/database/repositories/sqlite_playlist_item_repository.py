...
class SQLitePlaylistItemRepository(BaseRepository[PlaylistItemModel]):
    ...
    def add(self, playlist_item: PlaylistItemModel) -> PlaylistItemModel:
        ...
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?,?, ?)
            """,
            (playlist_item.playlist_item_id, playlist_item.channel_adder_id, 
             playlist_item.video_owner_channel_id, playlist_item.playlist_id, playlist_item.video_id,
             playlist_item.position, playlist_item.privacy_status, playlist_item.date_added)
            )
    ...
    def list_all(self, limit: Optional[int] = 2, sort_order: Optional[str] = 'ASC', 
                 sort_field: Optional[str] = 'id', offset: Optional[int] = 0) -> list[PlaylistItemModel]:
        cursor = self.connection.cursor()
        cursor.execute(
        f"""
         SELECT id, playlist_item_id, channel_adder_id, video_owner_channel_id, playlist_id, 
